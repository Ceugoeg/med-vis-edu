...
export class MediaPipeSensePublisher {
  ...
  onResults(results) {
    ...
    if (!handLandmarks || handLandmarks.length !== HAND_LANDMARK_COUNT) {
      ...
      this.openCandidateFrames = 0;
      this.forceFistStartMs = null;
      const state = this.stableState === 'FIST' ? 'FIST' : this.commitState('NONE');
    ...
    const holdFistCandidate = this.detectForceFistCandidate(smoothed);
    this.updateForceFistTimer(holdFistCandidate, nowMs);

    let rawState = this.detectGesture(smoothed);
    if (this.shouldForceFist(rawState, nowMs)) {
      rawState = 'FIST';
    }
  ...
  updateForceFistTimer(isCandidate, nowMs) {
    if (!isCandidate) {
      this.forceFistStartMs = null;
      return;
    }
    if (this.forceFistStartMs == null) {
      this.forceFistStartMs = nowMs;
    }
  }

  shouldForceFist(rawState, nowMs) {
    if (rawState !== 'NONE') return false;
    if (this.stableState !== 'NONE' && this.stableState !== 'FIST') return false;
    if (this.forceFistStartMs == null) return false;
    return nowMs - this.forceFistStartMs >= this.config.forceFistAfterMs;
  }

  detectForceFistCandidate(landmarks) {
    ...
    // 使用“到手腕距离压缩”做方向无关判定，降低对手掌朝向的敏感度。
    const looseCurlThreshold = this.config.fistExitCurlThreshold + 0.02;
    const curled = fingerPairs.map(([tipIndex, mcpIndex]) => {
      ...
      return delta < looseCurlThreshold;
    });

    const indexCurled = curled[0];
    const middleCurled = curled[1];
    const ringCurled = curled[2];
    const pinkyCurled = curled[3];
    const curledCountLoose = curled.filter(Boolean).length;

    // 食指/中指至少一根卷曲 + 四指里至少两根卷曲，即可作为强制 FIST 候选。
    // 不强依赖拇指，避免侧向握拳时被误杀。
    return (indexCurled || middleCurled) && curledCountLoose >= 2;
  }

  applyTransitionConstraints(rawState) {
    if (this.stableState === 'FIST' && rawState === 'NONE') {
      return 'FIST';
    }
  ...
  detectGesture(landmarks) {
    ...
    const fistYCountThreshold = isFistSticky ? 2 : 2;

    const fingerCurlFlags = fingerPairs.map(([tipIndex, mcpIndex]) => {
      const tipDist = distance3(landmarks[tipIndex], wrist);
      const mcpDist = distance3(landmarks[mcpIndex], wrist);
      const delta = tipDist - mcpDist;
      const yGapAbs = Math.abs(landmarks[tipIndex].y - landmarks[mcpIndex].y);
      return {
        curled: delta < fistDeltaThreshold || yGapAbs < fistYGapAbsThreshold,
      };
    });

    const indexCurled = fingerCurlFlags[0].curled;
    const middleCurled = fingerCurlFlags[1].curled;
    const ringCurled = fingerCurlFlags[2].curled;
    const pinkyCurled = fingerCurlFlags[3].curled;
    const coreFistReady = indexCurled && middleCurled && (ringCurled || pinkyCurled);

    const pinchSupportPairs = [
      [12, 9],
      [16, 13],
      [20, 17],
    ];
    const pinchSupportOpenCount = pinchSupportPairs.reduce((acc, [tipIndex, mcpIndex]) => {
      const tipDist = distance3(landmarks[tipIndex], wrist);
      const mcpDist = distance3(landmarks[mcpIndex], wrist);
      const delta = tipDist - mcpDist;
      const yGap = landmarks[tipIndex].y - landmarks[mcpIndex].y;
      return acc + (delta > openDeltaThreshold && yGap < openYGapThreshold ? 1 : 0);
    }, 0);
    const pinchSupportOpenThreshold = this.stableState === 'PINCH' ? 2 : 3;

    if (
      pinchMatched &&
      pinchBaselineReady &&
      pinchSupportOpenCount >= pinchSupportOpenThreshold
    ) {
      return 'PINCH';
    }
